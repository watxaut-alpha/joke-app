...
def put_validated_jokes_in_joke_db():
    ...
    sql_query = (
        "select * from validate_jokes where deleted_at is null and is_joke is true"
    )
    ...
    if not df_validated_jokes.empty:
        ...
        df_jokes = df_validated_jokes.drop(columns=col_exclude).rename(
            columns=col_rename
        )
        ...
        logger.info(
            "Updated '{}' jokes. New jokes in Jokes DB".format(len(df_jokes.index))
        )
